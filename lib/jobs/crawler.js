...
function getProductDetail(URL) {
    return new Promise((resolve, reject) => {
        ...
        .find('.page-title').set({
            'name': 'h1',
            'brand': 'a'
        }).find('.review:limit(1)').set({
            'sku': '#product-sku'
        }).find('.labelstatus').set({
            'availability': '.availability'
        }).find('.regular-price').set({
            'price': '.price'
        }).find('#product-description').set({
            'description': '.desc'
        }).find('.img-main').set({
            'imageSrc': '@src'
...
function getKidsPlazaDepartments() {
    return new Promise((resolve, reject) => {
        ...
        // .config('user_agent', randomUseragent.getRandom())
        .find('#mt_megamenu > .root + .level-0 > .megamenu-title').set({
            // 'title': 'span',
            'link': '@href'
...
function getKidsPlazaNextPage(departmentURL) {
    return new Promise((resolve, reject) => {
        let rs = [];
        osmosis.get(departmentURL).config('tries', 9)
        // .config('user_agent', randomUseragent.getRandom())
        .find('a + .next').set({
            'link': '@href'
        }).data(function (listing) {
